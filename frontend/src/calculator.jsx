...
function Calculator() {
    ...
    const [error, setError] = useState('');
    const [loading, setLoading] = useState(false); // Add loading state
    const [autoCalculate, setAutoCalculate] = useState(false); // Toggle for auto calculation
    ...
    return (
        <div className="fixed inset-0 flex justify-center items-center bg-gradient-to-br from-[#FF6B6B] via-[#4ECDC4] to-[#45B7D1]">
            <div className="w-full max-w-md mx-auto backdrop-blur-lg bg-white/20 p-4 rounded-3xl shadow-2xl border border-white/30 overflow-y-auto  max-h-[100vh]">
                {/* Display */}
                <div className="bg-white/30 rounded-2xl p-6 mb-6 backdrop-blur-md shadow-inner">
                    <input 
                        type="text" 
                        value={input} 
                        readOnly 
                        className="w-full bg-transparent text-gray-800 text-right text-lg mb-2 font-mono placeholder-gray-500"
                    ...
                    <input 
                        ...
                        value={loading ? "Calculating..." : result} 
                        readOnly 
                        className="w-full bg-transparent text-gray-900 text-right text-4xl font-bold font-mono placeholder-gray-500"
                        placeholder="0"
                    />
                </div>
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-3 gap-3">
                        <button 
                            ...
                            px-4 py-3 rounded-2xl text-sm font-medium transition-all duration-300 hover:shadow-lg transform hover:-translate-y-0.5`}
                        >
                            Auto: {autoCalculate ? 'ON' : 'OFF'}
                        </button>
                        <button 
                            onClick={toggleHistory}
                            className="bg-gradient-to-r from-[#4ECDC4] to-[#45B7D1] text-white px-4 py-3 rounded-2xl text-sm font-medium transition-all duration-300 hover:shadow-lg transform hover:-translate-y-0.5"
                        >
                            History
                        </button>
                        <button 
                            onClick={clearHistory}
                            className="bg-gradient-to-r from-[#FF69B4] to-[#FF1493] text-white px-4 py-3 rounded-2xl text-sm font-medium transition-all duration-300 hover:shadow-lg transform hover:-translate-y-0.5"
                        >
                            Clear
                        </button>
                    </div>
                    ...
                    {/* History Panel */}
                    {showHistory && (
                        <div className="bg-white/30 rounded-2xl p-4 backdrop-blur-md">
                            <div className="flex justify-between items-center mb-3">
                                <h2 className="text-gray-800 text-lg font-medium">History</h2>
                                <button 
                                    onClick={toggleHistory}
                                    className="text-[#FF1493] hover:text-[#FF69B4] text-sm"
                                >
                                    Close
                                </button>
                            </div>
                            <div className="max-h-48 overflow-y-auto space-y-2">
                                {history.map((item, index) => (
                                    <div 
                                        key={index}
                                        className="bg-white/40 p-3 rounded-xl text-gray-800 font-mono text-sm"
                                    >
                                        {item.expression} = {item.result}
                                    </div>
                                ))}
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        {['sin', 'cos', 'tan', 'log'].map(btn => (
                            <button
                                key={btn}
                                onClick={() => handleChange(btn)}
                                className="bg-gradient-to-r from-[#9C27B0] to-[#673AB7] text-white p-2 rounded-xl text-sm font-medium transition-all duration-300 hover:shadow-lg transform hover:-translate-y-0.5"
                            >
                                {btn}
                            </button>
                        ))}
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        {['7', '8', '9', '/', '4', '5', '6', '*', '1', '2', '3', '-', '.', '0', '=', '+'].map((btn) => (
                            <button
                                key={btn}
                                onClick={() => btn === '=' ? calculate(input) : handleChange(btn)}
                                className={`p-3 rounded-xl text-xl font-medium transition-all duration-300 hover:shadow-lg transform hover:-translate-y-0.5
                                    ${btn === '=' 
                                        ? 'bg-gradient-to-r from-[#4CAF50] to-[#45B7D1] text-white row-span-1' 
                                        : btn.match(/[0-9.]/) 
